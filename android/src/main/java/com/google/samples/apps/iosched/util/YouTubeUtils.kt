 ...
 */
object YouTubeUtils {
    private val TAG = makeLogTag(YouTubeUtils::class.java)

    @JvmStatic
    fun getYouTubeIdFromIdOrUrl(idOrUrl: String): String {
        val uri = Uri.parse(idOrUrl)
        if (uri.scheme != null) {
    ...
    fun showYouTubeVideo(videoId: String, activity: Activity) {
        if (!TextUtils.isEmpty(videoId)) {
            val liveIntent: Intent
            if (YouTubeIntents.isYouTubeInstalled(activity) && YouTubeApiServiceUtil.isYouTubeApiServiceAvailable(activity) == YouTubeInitializationResult.SUCCESS) {
                ...
                liveIntent = Intent(Intent.ACTION_VIEW)
                liveIntent.data = Uri.parse("https://www.youtube.com/watch?v=" + videoId)
            }
